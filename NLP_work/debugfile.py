...
def train(net, train_iter, val_iter,num_epochs, device=torch.device('cpu'), save_path=None):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_iter):
            ...
            if (i + 1) % (train_num_batches // 5) == 0 or i == train_num_batches - 1:
                animator.add(epoch + (i + 1) / train_num_batches, (train_metric[0] / train_metric[1],))
...
if __name__ == '__main__':
    ...
    train(net, data_iter,val_iter, num_epochs, save_path='model_epoch1.pt')
    get_similar_tokens('chip', 3, net[0])


    path_test = r'C:\Users\fulian\Desktop\NLP_work\data\ptb.test.txt'
    x_vals, y_vals, labels = reduce_dimensions(path_train,vocab,data_iter,net[0])
